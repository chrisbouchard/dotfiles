...
syn keyword psqlKeyword	access add after as asc before begin by case check cluster
syn keyword psqlKeyword	column compress connect current cursor decimal default
syn keyword psqlKeyword	desc each else elsif end exception exclusive file for from
...
syn keyword psqlKeyword	public resource return returns row rowlabel rownum rows
...
syn keyword psqlKeyword	language handler trusted untrusted validator schema
syn keyword psqlKeyword cascade restrict no action new old tg_name tg_when
syn keyword psqlKeyword tg_level tg_op tg_relid tg_relname tg_table_name
syn keyword psqlKeyword tg_table_schema tg_nargs tg_argv raise trigger

iabb <buffer> access ACCESS
iabb <buffer> add ADD
...
iabb <buffer> cluster CLUSTER
iabb <buffer> column COLUMN
iabb <buffer> compress COMPRESS
iabb <buffer> connect CONNECT
iabb <buffer> current CURRENT
...
iabb <buffer> elsif ELSIF
iabb <buffer> end END
iabb <buffer> exception EXCEPTION
iabb <buffer> exclusive EXCLUSIVE
iabb <buffer> file FILE
iabb <buffer> for FOR
iabb <buffer> from FROM
iabb <buffer> function FUNCTION
iabb <buffer> group GROUP
iabb <buffer> having HAVING
iabb <buffer> identified IDENTIFIED
iabb <buffer> if IF
iabb <buffer> immediate IMMEDIATE
iabb <buffer> increment INCREMENT
iabb <buffer> index INDEX
iabb <buffer> initial INITIAL
iabb <buffer> into INTO
iabb <buffer> is IS
iabb <buffer> level LEVEL
iabb <buffer> loop LOOP
iabb <buffer> maxextents MAXEXTENTS
iabb <buffer> mode MODE
iabb <buffer> modify MODIFY
iabb <buffer> nocompress NOCOMPRESS
iabb <buffer> nowait NOWAIT
iabb <buffer> of OF
iabb <buffer> offline OFFLINE
iabb <buffer> on ON
iabb <buffer> online ONLINE
iabb <buffer> start START
iabb <buffer> successful SUCCESSFUL
iabb <buffer> synonym SYNONYM
...
iabb <buffer> user USER
iabb <buffer> validate VALIDATE
iabb <buffer> values VALUES
iabb <buffer> view VIEW
iabb <buffer> whenever WHENEVER
iabb <buffer> where WHERE
iabb <buffer> with WITH
iabb <buffer> option OPTION
iabb <buffer> order ORDER
iabb <buffer> privilages PRIVILAGES
iabb <buffer> procedural PROCEDURAL
iabb <buffer> public PUBLIC
iabb <buffer> resource RESOURCE
iabb <buffer> return RETURN
iabb <buffer> returns RETURNS
iabb <buffer> row ROW
iabb <buffer> rowlabel ROWLABEL
iabb <buffer> rownum ROWNUM
iabb <buffer> rows ROWS
iabb <buffer> session SESSION
iabb <buffer> share SHARE
iabb <buffer> size SIZE
iabb <buffer> type TYPE
iabb <buffer> using USING
iabb <buffer> close CLOSE
iabb <buffer> open OPEN
iabb <buffer> declare DECLARE
iabb <buffer> constraint CONSTRAINT
iabb <buffer> join JOIN
iabb <buffer> temporary TEMPORARY
iabb <buffer> sequence SEQUENCE
iabb <buffer> inner INNER
iabb <buffer> outer OUTER
iabb <buffer> cross CROSS
iabb <buffer> natural NATURAL
iabb <buffer> full FULL
iabb <buffer> left LEFT
iabb <buffer> right RIGHT
iabb <buffer> language LANGUAGE
iabb <buffer> handler HANDLER
iabb <buffer> trusted TRUSTED
iabb <buffer> untrusted UNTRUSTED
iabb <buffer> validator VALIDATOR
...
iabb <buffer> new NEW
iabb <buffer> old OLD
iabb <buffer> tg_name TG_NAME
iabb <buffer> tg_when TG_WHEN
iabb <buffer> tg_level TG_LEVEL
iabb <buffer> tg_op TG_OP
iabb <buffer> tg_relid TG_RELID
iabb <buffer> tg_relname TG_RELNAME
iabb <buffer> tg_table_name TG_TABLE_NAME
iabb <buffer> tg_table_schema TG_TABLE_SCHEMA
iabb <buffer> tg_nargs TG_NARGS
iabb <buffer> tg_argv TG_ARGV
iabb <buffer> raise RAISE
iabb <buffer> trigger TRIGGER


syn keyword psqlOperator	not and or primary foreign key
...
iabb <buffer> in IN
iabb <buffer> any ANY
iabb <buffer> some SOME
iabb <buffer> all ALL
iabb <buffer> between BETWEEN
iabb <buffer> exists EXISTS
iabb <buffer> ilike ILIKE
iabb <buffer> like LIKE
iabb <buffer> escape ESCAPE
iabb <buffer> cast CAST
iabb <buffer> union UNION
iabb <buffer> intersect INTERSECT
iabb <buffer> minus MINUS
iabb <buffer> prior PRIOR
...
iabb <buffer> audit AUDIT
iabb <buffer> comment COMMENT
iabb <buffer> commit COMMIT
iabb <buffer> create CREATE
iabb <buffer> delete DELETE
iabb <buffer> drop DROP
iabb <buffer> execute EXECUTE
iabb <buffer> explain EXPLAIN
iabb <buffer> grant GRANT
iabb <buffer> insert INSERT
iabb <buffer> lock LOCK
iabb <buffer> rename RENAME
iabb <buffer> revoke REVOKE
iabb <buffer> rollback ROLLBACK
iabb <buffer> savepoint SAVEPOINT
iabb <buffer> select SELECT
iabb <buffer> set SET
iabb <buffer> truncate TRUNCATE
iabb <buffer> update UPDATE
iabb <buffer> vacuum VACUUM
...
iabb <buffer> long LONG
iabb <buffer> mlslabel MLSLABEL
iabb <buffer> number NUMBER
iabb <buffer> raw RAW
iabb <buffer> rowide ROWID
iabb <buffer> varchar VARCHAR
iabb <buffer> varchar2 VARCHAR2
iabb <buffer> varray VARRAY
iabb <buffer> int INT
iabb <buffer> int4 INT4
iabb <buffer> int8 INT8
iabb <buffer> float4 FLOAT4
iabb <buffer> float8 FLOAT8
iabb <buffer> line LINE
iabb <buffer> interval INTERVAL
iabb <buffer> lseq LSEQ
iabb <buffer> smallint SMALLINT
iabb <buffer> bigint BIGINT
iabb <buffer> serial SERIAL
iabb <buffer> bigserial BIGSERIAL
iabb <buffer> inet INET
iabb <buffer> cidr CIDR
iabb <buffer> macaddr MACADDR
iabb <buffer> bit BIT
iabb <buffer> varbit VARBIT
iabb <buffer> bool BOOL
iabb <buffer> box BOX
iabb <buffer> bytea BYTEA
iabb <buffer> circle CIRCLE
iabb <buffer> numeric NUMERIC
iabb <buffer> decimal DECIMAL
iabb <buffer> path PATH
iabb <buffer> point POINT
iabb <buffer> polygon POLYGON
iabb <buffer> text TEXT
iabb <buffer> time TIME
iabb <buffer> timetz TIMETZ
iabb <buffer> timestamp TIMESTAMP
iabb <buffer> timestamptz TIMESTAMPTZ
iabb <buffer> real REAL
iabb <buffer> name NAME
iabb <buffer> int2vector INT2VECTOR
iabb <buffer> regproc REGPROC
iabb <buffer> oid OID
iabb <buffer> tid TID
iabb <buffer> xid XID
iabb <buffer> cid CID
iabb <buffer> oidvector OIDVECTOR
iabb <buffer> smgr SMGR
iabb <buffer> lseg LSEG
iabb <buffer> double DOUBLE
iabb <buffer> abstime ABSTIME
iabb <buffer> reltime RELTIME
iabb <buffer> tinterval TINTERVAL
iabb <buffer> aclitem ACLITEM
iabb <buffer> void VOID
iabb <buffer> opaque OPAQUE
iabb <buffer> refcursor REFCURSOR
iabb <buffer> regprocedure REGPROCEDURE
iabb <buffer> regoper REGOPER
iabb <buffer> regoperator REGOPERATOR
iabb <buffer> regclass REGCLASS
iabb <buffer> regtype REGTYPE
iabb <buffer> record RECORD
iabb <buffer> cstring CSTRING
iabb <buffer> anyarray ANYARRAY
iabb <buffer> array ARRAY
iabb <buffer> language_handler LANGUAGE_HANDLER
